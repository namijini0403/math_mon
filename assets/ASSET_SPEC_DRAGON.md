...
미니미 스타일 투명 PNG 512×512. **부재 시 이모지 폴백이라 블로커 아님.** 파일명이 코드와 일치하면 자동 반영.

**레어(각성) 4종 — `client/public/assets/dragon/rare/{id}.png`** (한 학기 ~6보스 봉인 + 뚜렷한 으뜸 속성)
| 파일 | 이름 | 컨셉 |
|---|---|---|
| `rare-sun.png` | 작열하는 태양 패왕룡 | 황금 불꽃 갈기, 태양 문장, 더 웅장하고 화려한 각성형(폭력적이지 않게) |
| `rare-moon.png` | 몽환의 월령 마도룡 | 보랏빛 오로라 날개, 초승달 룬이 떠다니는 신비형 |
| `rare-star.png` | 천공의 성좌룡 | 몸에 별자리가 흐르는 푸른 우주룡, 빛나는 성운 |
| `rare-forest.png` | 세계수의 정령룡 | 비취 비늘에 꽃·잎이 만개, 작은 정령들이 함께 |

**슈퍼레어(전설) 2종 — `client/public/assets/dragon/super/{id}.png`** (두 학기 ~12보스 봉인)
| 파일 | 이름 | 도달 조건 | 컨셉 |
|---|---|---|---|
| `super-rainbow.png` | 무지개 창세룡 | 네 속성 고루 높음(균형) | 무지개빛 오로라를 두른 창세의 용, 사방에 빛 |
| `super-obsidian.png` | 흑요석 폭풍룡 | 한 속성 극단 편향 | 흑요석 비늘에 폭풍 번개를 두른 장엄한 용(무섭지 않게, 멋짐 강조) |
