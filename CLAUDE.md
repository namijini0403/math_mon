...
- 테스트 수 기준선: 1485개(2026-06-14: 1472 + curriculumGuard 13). 줄어들면 뭔가 깨진 것.
- 학습강화 Phase B(풀이 보기, 진행 중): 정답에도 풀이 노출=components/SolutionReveal.tsx(💡 접이식, 5개 풀이화면 정답분기에 연결). 풀이 표기 교육과정 가드=game/curriculumGuard.ts(학년별 금지표기 ×<2·÷<3·분수<3·소수<3·숫자거듭제곱). **AUDITED_UNITS에 등록된 단원만 위반=실패로 강제** — 1단원부터 순차 audit하며 추가. 미등록 단원의 알려진 위반은 docs/curriculum/notation-violations.md. 학년별 누적 교육과정 기준=docs/curriculum/g{N}.md. ⚠️1·2학년 풀이는 부등호 기호 대신 '말'로(도입학년 불명). 완료: unitNum9(1-1①).
